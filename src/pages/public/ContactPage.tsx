          ...
          <div className="bg-neutral-100 rounded-3xl overflow-hidden h-[300px] border border-neutral-200 flex items-center justify-center relative">
            {settings.contact_maps_iframe ? (
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: settings.contact_maps_iframe }}
              />
            ) : (
              <>
                <div className="text-center p-6 z-10">
                  <MapPin size={48} className="mx-auto text-green-600 mb-4 opacity-50" />
                  <p className="text-gray-500 font-medium">Peta Lokasi Pabrik (Google Maps)</p>
                </div>
                <div className="absolute inset-0 opacity-10 bg-[url('https://maps.googleapis.com/maps/api/staticmap?center=-6.89,110.63&zoom=12&size=600x300&key=placeholder')] bg-cover"></div>
              </>
            )}
          </div>
        </div>
